<template>
  <details class="property-section control-properties-section" open>
    ...
    <div v-for="field in definition.fields" :key="field.key" class="control-field" :class="`kind-${field.kind}`" :title="field.description">
      <div class="field-heading">
        <span>{{ field.label }}</span>
        <small v-if="field.tweenable">Tween</small>
        <small v-if="field.runtimeReadOnly" class="readonly-badge">运行时只读</small>
      </div>

      <ColorRGBAField v-if="field.kind === 'color'" :label="field.label" :model-value="asColor(modelValue[field.key])" @update:model-value="updateField(field.key, $event)" />
      ...
      <ScrubbableNumberInput v-else-if="field.kind === 'number'" :aria-label="field.label" :model-value="numberValue(modelValue[field.key])" :min="field.min" :max="field.max" :step="field.step ?? 1" allow-empty placeholder="未设置" @update:model-value="updateNumber(field.key, $event)" />
...
import type { ControlDefinition } from "./controlRegistry";
import type { ColorRGBA, ControlType } from "./types";

const props = defineProps<{ definition: ControlDefinition<ControlType>; modelValue: Record<string, unknown> }>();
