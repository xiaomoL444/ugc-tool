...
<template>
  <div class="camera-editor">
    <div v-if="shot" class="camera-basics">
      <label>相机<input :value="values.cameraName" placeholder="Default" @input="update('cameraName', ($event.target as HTMLInputElement).value)" /></label>
    </div>
    <template v-if="shot">
...
<style scoped>
.camera-editor { color: #cdd9e8; font-size: 12px; }
.camera-basics { display: grid; grid-template-columns: minmax(0, 1fr); }
.camera-basics label { display: grid; gap: 7px; color: #91a4bc; font-size: 11px; }
.camera-basics input { box-sizing: border-box; min-width: 0; width: 100%; padding: 9px 10px; color: #e4edfa; background: #151e2c; border: 1px solid #35455c; border-radius: 7px; font: inherit; }
.motion-tabs { display: flex; gap: 6px; margin-top: 16px; padding: 4px; border-radius: 9px; background: #111b29; }
.motion-tabs button { display: flex; flex: 1; align-items: center; justify-content: space-between; padding: 10px 12px; border: 0; border-radius: 6px; background: transparent; color: #8da1bb; cursor: pointer; }
.motion-tabs button[aria-pressed="true"] { background: #2c405b; color: #f0f5ff; box-shadow: 0 2px 5px #0002; }
.motion-tabs small { color: #93b6df; font-size: 10px; }
.viewpoint-toggle { display: flex; align-items: center; gap: 8px; margin: 16px 0; color: #cdd9e8; font-size: 12px; }
.viewpoint-toggle input { width: 14px; height: 14px; padding: 0; margin: 0; accent-color: #74aafa; }
.viewpoint-toggle small { margin-left: auto; color: #7d93af; font-size: 10px; }
.viewpoint-empty { padding: 16px; border: 1px dashed #35455c; border-radius: 8px; color: #91a4bd; font-size: 12px; line-height: 1.7; }
.camera-editor :deep(button) { font-family: inherit; }
.camera-editor :deep(button:focus-visible), .camera-editor input:focus-visible { outline: 2px solid #83b4ff; outline-offset: 2px; }
.camera-editor :deep(.motion-modes) { display: flex; gap: 6px; margin: 14px 0 20px; }
.camera-editor :deep(.motion-modes button) { flex: 1; min-width: 0; display: grid; gap: 5px; padding: 9px 2px; border: 1px solid #35455c; border-radius: 7px; background: transparent; color: #a6b6cc; cursor: pointer; }
.camera-editor :deep(.motion-modes button[aria-pressed="true"]) { background: #253e5d; border-color: #73a7e9; color: #dcecff; }
...
.camera-editor :deep(.slot-heading small) { color: #7d93af; margin-left: auto; }
...
.camera-editor :deep(.camera-motion input:not([type="checkbox"])), .camera-editor :deep(.camera-motion select) { padding: 8px; border-radius: 6px; border-color: #36475f; background: #121d2c; }
.camera-editor :deep(.camera-motion .vector-axes label) { background: #121d2c; border: 1px solid #36475f; border-radius: 6px; padding-left: 8px; gap: 2px; }
.camera-editor :deep(.camera-motion .vector-axes input) { border: 0; background: transparent; padding: 8px 4px; }
.camera-editor :deep(.camera-motion input[type="checkbox"]) { width: 14px; height: 14px; padding: 0; flex: 0 0 14px; accent-color: #74aafa; }
.camera-editor :deep(.add-target), .restore-shot { width: 100%; padding: 9px; margin-top: 8px; border: 1px dashed #526c8d; border-radius: 7px; background: transparent; color: #a6c8ef; cursor: pointer; }
.camera-editor :deep(.motion-extras) { display: grid; gap: 14px; margin-top: 14px; padding: 12px; border-radius: 8px; background: #243249; }
.camera-editor :deep(.camera-notice) { color: #e9be7d; font-size: 11px; line-height: 1.6; }
.camera-advanced { margin-top: 22px; border-top: 1px solid #354359; padding-top: 13px; }
.camera-advanced > summary { cursor: pointer; color: #91a4bd; font-size: 11px; }
.camera-advanced > summary span { margin-left: 10px; color: #607891; font-size: 10px; }
</style>
