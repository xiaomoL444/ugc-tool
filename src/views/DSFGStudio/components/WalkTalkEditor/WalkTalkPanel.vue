      ...
      <article v-for="(entry, index) in project.entries" :key="entry.id" class="dialogue-card" :aria-label="`第 ${index + 1} 条台词`">
        ...
        <div class="card-body">
          ...
          <div class="content-fields">
            <label class="content-label">台词内容 <code>content</code><textarea v-model="entry.content" :aria-label="`第 ${index + 1} 条内容`" rows="4" placeholder="填写台词内容…" /></label>
            <div class="timing-fields">
              <label>推进延迟 <span class="delay-input"><input v-model="entry.continueDelay" inputmode="decimal" :aria-label="`第 ${index + 1} 条推进延迟`" /><span>秒</span></span></label>
            </div>
          </div>
        </div>
...
<style scoped>
.walk-talk-panel { display: flex; flex-direction: column; flex: 1; min-height: 0; min-width: 0; background: #f5f8fc; color: #34445b; font-size: 13px; text-align: left; }
...
.dialogue-list { flex: 1; min-height: 0; overflow: auto; padding: 16px; scrollbar-gutter: stable; }
.dialogue-card { border: 1px solid #d7e1ee; background: white; border-radius: 10px; margin-bottom: 16px; overflow: hidden; box-shadow: 0 2px 6px #31557d05; }
.dialogue-card:focus-within { border-color: #a5bedf; }
.card-header { display: flex; align-items: center; flex-wrap: wrap; gap: 10px; padding: 10px 16px; background: #f0f5fc; border-bottom: 1px solid #e2eaf4; }
...
.card-title { flex: 1; min-width: 90px; overflow-wrap: anywhere; text-align: left; font-weight: 600; }
.card-title small { color: #8292a7; }
.card-body { display: grid; grid-template-columns: minmax(180px, 240px) minmax(0, 1fr); gap: 20px; padding: 16px; }
label { display: block; min-width: 0; color: #61748c; font-size: 12px; text-align: left; }
...
.speaker-fields { display: flex; flex-direction: column; gap: 10px; padding-right: 20px; border-right: 1px solid #edf1f6; }
.content-fields { display: flex; flex-direction: column; min-width: 0; gap: 12px; }
.content-label { display: flex; flex-wrap: wrap; align-content: flex-start; align-items: baseline; column-gap: 5px; flex: 1; }
.content-fields > label textarea { flex-basis: 100%; min-height: 112px; font-size: 14px; line-height: 1.7; }
.timing-fields { display: flex; flex-wrap: wrap; gap: 12px; }
.timing-fields label { display: flex; align-items: center; gap: 12px; }
.delay-input { display: flex; align-items: center; gap: 8px; color: #8393a7; }
.delay-input input { width: 100px; margin: 0; }
.params { margin: 0 16px 14px; padding: 10px 12px; border: 1px solid #edf1f6; border-radius: 6px; background: #f8fafc; text-align: left; }
...
@media (max-width: 850px) { .card-body { grid-template-columns: 1fr; gap: 14px; } .speaker-fields { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); padding: 0 0 14px; border-right: 0; border-bottom: 1px solid #edf1f6; } .speaker-fields label:last-child { grid-column: 1 / -1; } }
@media (max-width: 540px) { .dialogue-list { padding: 10px; } .card-body { padding: 12px; } .card-actions { flex-basis: 100%; justify-content: flex-end; } .speaker-fields { grid-template-columns: minmax(0, 1fr); } .params { margin-inline: 12px; } }
</style>
